...
final class Instance implements Validation
{
    ...
    public function validate(mixed $value, ValidationStrategy $failure): void
    {
        ...
        if (!is_object($value)) {
            $failure->setFailure($this->message ?? var_export($value, true) . ' must be an object-instance of ' . $this->class . ' in {path}');

            return;
        }

        if (!($value instanceof $this->class)) {
            $failure->setFailure($this->message ?? var_export($value, true) . ' is not an instance of ' . $this->class . ' in {path}');
        }
    }
}
